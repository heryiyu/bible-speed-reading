...
- **Edge Functions** (`supabase/functions/`): six total, not two.
  ...
  - `issue-report-maintenance` — daily cron: auto-closes issue-report threads 14 days after an admin reply with no response, purges threads closed 180+ days plus orphaned screenshots. Guarded by a shared-secret header (`x-maintenance-secret`), not a Logto/Supabase token.
