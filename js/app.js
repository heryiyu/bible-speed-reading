...
import '../config.js';
import './data/bible_data.js?v=20260918_optimistic_boot_shell';
import './data/bible_verse_counts.js';
import './copy/zh-Hant.js?v=20260918_optimistic_boot_shell';
import './data/church_campaign.js?v=20260918_optimistic_boot_shell';
import './design/design-tokens.js';
import './design/design-system-helpers.js?v=20260918_optimistic_boot_shell';
import './design/icon-registry.js?v=20260918_optimistic_boot_shell';
...
import './db.js?v=20260919_daily_quiz_stats_rpc';
...
} from './login-onboarding-gate.mjs?v=20260918_optimistic_boot_shell';
import { cleanupProductionStorage } from './production-cleanup.mjs';
import { initializePwa } from './pwa/PwaCoordinator.js?v=20260918_optimistic_boot_shell';
...
import { SupabaseRepository } from './pwa/SupabaseRepository.js?v=20260918_optimistic_boot_shell';
...
const moduleCache = {};
const RELEASE_ONBOARDING_MODULE_PATH = './modules/onboarding-helper.js?v=20260918_optimistic_boot_shell';
...
appRouter.switchTab = async function (tabId, options = {}) {
  ...
  try {
    // ── Pre-flight: leaving Plan tab — resync the shared state.activePlan
    // pointer. Opening an ended plan's detail view or previewing an unjoined
    // devotional/group-meeting plan overwrites this single pointer, and
    // nothing else resets it; without this, Home keeps rendering whatever
    // plan was last viewed instead of the user's actual current plan.
    if (previousTab === "plan-view" && tabId !== "plan-view" && typeof window.selectMostRecentActivePlan === "function") {
      state.activePlan = window.selectMostRecentActivePlan(state.activePlans);
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (state.optimisticBootPending) {
      // Optimistic boot (see db.js init()): db.init() already returned true
      // from a cached identity, not a confirmed live sync, so
      // initialSessionSyncSucceeded isn't trustworthy yet for gating the
      // onboarding modal. Defer the check to whenever the background
      // reconciliation actually settles, without blocking anything above
      // this line on it.
      window.addEventListener("auth:background-sync-settled", (event) => {
        maybeShowReleaseOnboarding({
          auth,
          syncComplete: Boolean(event.detail?.success) && initialDataLoadSucceeded === true,
